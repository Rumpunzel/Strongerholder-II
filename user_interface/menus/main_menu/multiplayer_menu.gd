...
func player_name_changed(player_name: String) -> void:
	if _player_name.text == player_name: return
	_player_name.text = player_name

func game_hosted(host_ip_address: StringName, _port: int) -> void:
	_host_ip_address_button.text = host_ip_address

func stopped_hosting_game() -> void:
	_host_ip_address_button.text = ""

func disconnected_from_multiplayer() -> void:
