...
func _on_game_joined(host_player_info: Dictionary) -> void:
	PlayerSynchronizer.validate_player_info(host_player_info)
...
func _on_player_joined(player: PlayerSynchronizer) -> void:
	toast_success("%s joined!" % player.player_name)

func _on_player_disconnected(_peer_id: int, player: PlayerSynchronizer) -> void:
