...
func collect_properties() -> Dictionary:
	assert(_multiplayer_synchronizer)
	var _properties_to_serialize := _multiplayer_synchronizer.replication_config.get_properties()
	var root_node_path := _multiplayer_synchronizer.root_path
	var root_node := get_node(root_node_path)
	
	var properties_dict := { }
	for property_path: NodePath in _properties_to_serialize:
...
func parse_properties(collected_properties: Dictionary) -> void:
	assert(_multiplayer_synchronizer)
	var root_node_path := _multiplayer_synchronizer.root_path
	var root_node := get_node(root_node_path)
...
func deserialize(serialized_dict: String) -> void:
	var collected_properties: Dictionary = Serialization.decode_data(serialized_dict)
	assert(collected_properties is Dictionary)
