...
func serialize(save_file_path: StringName) -> Error:
	...
	var serialized_nodes: String = SerializerNode.encode_data(collected_nodes)
...
func deserialize(save_file_path: StringName) -> Error:
	...
	var collected_nodes: Dictionary[StringName, Array] = SerializerNode.decode_data(serialized_nodes)
	assert(collected_nodes is Dictionary[StringName, Array])
