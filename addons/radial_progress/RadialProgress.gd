...
func animate(duration: float, clockwise: bool = true, initial_value: float = 0.0) -> void:
	var from: float = initial_value if clockwise else max_value
	var to: float = max_value if clockwise else initial_value
	var tween: Tween = create_tween()
	tween.tween_property(self, "progress", to, duration).from(from).set_trans(Tween.TRANS_LINEAR).set_ease(Tween.EASE_IN)
...
func draw_circle_arc(center: Vector2, radius: float, angle_from: float, angle_to: float, color: Color) -> void:
	...
	var start_angle: float = angle_from - (PI / 2.0)
	#var end_angle: float = angle_to - (PI / 2.0)
	var segment_angle: float = TAU / float(nb_points)
	var progress_points: float = (angle_to - angle_from) / segment_angle
	for i in range(progress_points + 1):
		var angle_point: float = start_angle + float(i) * segment_angle
		points_arc.push_back(center + Vector2(cos(angle_point), sin(angle_point)) * radius)
	if points_arc.size() <= 2: return
	draw_polygon(points_arc, colors)

func draw_ring_arc(center: Vector2, radius1: float, radius2: float, angle_from: float, angle_to: float, color: Color) -> void:
	var points_arc := PackedVector2Array()
	var colors := PackedColorArray([color])
	var start_angle: float = angle_from - (PI / 2.0)
	#var end_angle: float = angle_to - (PI / 2.0)
	...
	for i in range(progress_points, -1, -1):
		var angle_point: float = start_angle + float(i) * segment_angle
