...
@onready var _lobby: Lobby = _initialize_lobby()
@onready var _main_menu: MainMenu = _initialize_main_menu()
...
func host_game() -> void:
	...
	_main_menu.multiplayer_menu.game_hosted(DEFAULT_SERVER_IP, PORT)
	_unpause_game()
...
func stop_hosting_game() -> void:
	...
	_main_menu.multiplayer_menu.stopped_hosting_game()

func leave_game() -> void:
	_lobby.leave_multiplayer()
	_main_menu.multiplayer_menu.disconnected_from_multiplayer()
	request_load()

static func _initialize_lobby() -> Lobby:
	var lobby_scene: PackedScene = preload("uid://citi18cutmbiw")
	var lobby: Lobby = lobby_scene.instantiate()
	Game.add_child(lobby)
	return lobby

static func _initialize_main_menu() -> MainMenu:
	var main_menu_scene: PackedScene = preload("uid://5sd2jgms4uso")
	var main_menu: MainMenu = main_menu_scene.instantiate()
	main_menu.multiplayer_menu.player_name_changed(Game.player_name)
